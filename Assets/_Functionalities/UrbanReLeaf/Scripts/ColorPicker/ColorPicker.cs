...
namespace Netherlands3D.Functionalities.UrbanReLeaf
{
    ...
    public class ColorPicker : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
    {
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            {
                h = (Mathf.Atan2(pos.y, pos.x) * recip2Pi + 1) % 1;
                ApplyColor();
            }
            ...
            if (pointerDownLocation == PointerDownLocation.SVSquare)
            {
                ...
                s = Mathf.InverseLerp(-size, size, pos.x);
                v = Mathf.InverseLerp(-size, size, pos.y);
                ApplyColor();
            }
        }
        ...
        {
            var rect = rectTransform.rect;
            var smallestDimension = Mathf.Min(rect.width, rect.height);
