...
namespace Netherlands3D.Twin
{
    public class ATMTileCoordinates : MonoBehaviour
    {
        ...
        public static Vector2Int originOffset = new Vector2Int(419, -1618);
        public string GetTileUrl(Coordinate coord, int zoomLevel)
        {
            ...
            // Debug.LogError(a + "-----" + b);
            // CalculateTileOffset();
            var offsetTileXY = OffsetTileXY(converted.x, converted.y);
            return GetTileUrl(14, offsetTileXY.x, offsetTileXY.y);
        }

        public static (int x, int y) OffsetTileXY(int x, int y)
        {
            x -= originOffset.x;
            y -= originOffset.y;
            return (x, y);
        ...
        public void CalculateTileOffset()
        {
            var coord = new Coordinate(CoordinateSystem.RD, 121950, 486054);
            var converted = CoordinateToTileXY(coord, 19);
            converted.x -= 3788*2+2;//7576/8620.png
            converted.y -= 4310*2-1;
            originOffset = new Vector2Int(converted.x, converted.y);
