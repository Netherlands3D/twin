...
{
    public static class LnOperator
    {
        ...
        public static double Evaluate(Expression expr, ExpressionContext ctx)
        {
            ...
            if (!ExpressionEvaluator.IsNumber(o))
                throw new InvalidOperationException($"\"ln\" requires a numeric operand, got {o?.GetType().Name}");
            return Math.Log(Convert.ToDouble(o, CultureInfo.InvariantCulture));
        }
    }
}