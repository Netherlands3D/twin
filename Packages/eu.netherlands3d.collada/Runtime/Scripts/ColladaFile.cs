...
namespace Netherlands3D.Collada
{
	...
	public class ColladaFile
	{
		...
		private Dictionary<string, List<double[]>> objectsTriangles;

		private bool generateGeoHeader = false;
		private double longitude = 0;
		private double latitude = 0;
		private double altitudeFromGround = 0;

		private bool finishedDocument = false;

		/// <summary>
		/// Add a new mesh object using a list of vertices
		/// </summary>
		/// <param name="triangleVertices">Vertices as double[3]</param>
		public void AddObjectTriangles(List<double[]> triangleVertices, string objectName, Material objectMaterial)
		{
			if (objectsTriangles == null) objectsTriangles = new Dictionary<string, List<double[]>>();
		...
		/// <summary>
		/// Adds georeference to collada.
		/// This is not officialy supported in the 1.4 version of collada, but is supported by Sketchup Pro 2019 and Blender 2.92
		/// </summary>
		public void SetGeoReference(double longitude, double latitude, double altitudeFromGround)
		{
		...
		public void Finish(Formatting formatting = Formatting.Indented)
		{
			...
			if (longitude > 0 && latitude > 0 && altitudeFromGround > 0)
			{
				...
				writer.WriteString(longitude.ToString(CultureInfo.InvariantCulture));
				writer.WriteEndElement(); //end longitude
				writer.WriteStartElement("latitude");
				writer.WriteString(latitude.ToString(CultureInfo.InvariantCulture));
				writer.WriteEndElement(); //end latitude
				writer.WriteStartElement("altitude");
				writer.WriteAttributeString("mode", "relativeToGround");
				writer.WriteString(altitudeFromGround.ToString(CultureInfo.InvariantCulture));
		...
		private void WriteAssetHeader()
		{
			...
			writer.WriteString("Netherlands3D");
		...
		private void WriteGeometry(KeyValuePair<string, List<double[]>> meshNameAndVerts, bool writeVertexNormals = false)
		{
			...
			for (int i = 0; i < meshVertices.Count; i++)
			{
				...
				writer.WriteString(vert[0] + " " + vert[1] + " " + vert[2]);
		...
		public void Save(string filePath = "")
		{
			...
			{
				Debug.Log(filePath);
				File.WriteAllText(filePath, GetColladaXML(), Encoding.UTF8);
				stringWriter = null;
				writer = null;
				return;
			}
