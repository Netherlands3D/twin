...
namespace Netherlands.Indicators
{
    public class FeatureCollectionVisualiser : MonoBehaviour
    {
        ...
        {
            string id = feature.Id // Use the official ID 
                ?? feature.Properties["ID"].ToString() // .. or the unofficial ID if it exists
                ?? feature.Properties["OBJECTID"].ToString() // .. or the autogenerated objectid if it exists
                ?? ("Feature" + featureIndex); // Supplier doesn't care, so we come up with an id
