...
* The areaselection for grids now inherits from polygon input. 
* Added an extra state to drawmode, "Selected" to indicate that the area is selected but not being edited.
...
* Including closing edge to polygon isclockwise check
...
* Now also correctly invert the triangle order when the winding order is inverted
