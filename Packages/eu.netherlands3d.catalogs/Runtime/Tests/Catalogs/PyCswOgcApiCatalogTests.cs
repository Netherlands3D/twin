...
namespace Netherlands3D.Catalogs.Catalogs
{
    ...
    public class PyCswOgcApiCatalogTests
    {
        ...
        public async Task BrowseAsync_DefaultPage_ReturnsAllTopLevelItems()
        {
            ...
            Assert.AreEqual("pycsw Geospatial Catalogue gisdata demo", itemsAsList[0].Title);
        ...
        public async Task BrowseAsync_GetItemsFromMainCollection()
        {
            ...
            Assert.AreEqual(50, mainCollectionItemsAsList.Count);
            Assert.IsInstanceOf<FolderItem>(firstItem);
            
            Assert.IsTrue(firstItem.HasNextPage);
            Assert.IsTrue(firstItem.IsFirstPage);
            Assert.IsFalse(firstItem.HasPreviousPage);
            Assert.IsFalse(firstItem.IsLastPage);
        }
